...
clf()
h = hist(log10.(rel_error1), collect(-16 : 0.5 : -8); align="mid", alpha=0.5 )
ylabel("number")
...
for i=1:ms
    ...
    if length(k) > 0
        ...
        elseif i==4
            ...
            n_series = (i - floor(i,digits=-1))
            m = floor( i/ 10)
            plot(real.(z[k]), imag.(z[k]), "."; markersize=markerSize, label=@sprintf("Series %d used %d times", n_series, m))
        end
    end
end
legend()
