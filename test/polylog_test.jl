...
@testset "Polylogarithm polylog" begin
    ...
    @testset "Test s = n (a real integer)" begin
        ...
        @testset "   dilogarithm for real z" begin
            ...
            @test SF.polylog(2, -φ)     ≈ -pi^2/10 - log(φ)^2
            @test SF.polylog(2, -1/φ)   ≈ -pi^2/15 + log(φ)^2/2
            @test SF.polylog(2, 1/φ^2)  ≈  pi^2/15 - log(φ)^2
            @test SF.polylog(2, 1/φ)    ≈  pi^2/10 - log(φ)^2
            @test SF.polylog(2, φ)      ≈  11*pi^2/15 + log(Complex(-1/φ))^2/2 # wiki has this one, but no ref
            @test SF.polylog(2, φ^2)    ≈ -11*pi^2/15 - log(Complex(-φ))^2
        ...
        @testset "   trilogarithm for real z" begin
            ...
            @test SF.polylog(3, Float64(φ)^(-2)) ≈ 4*SF.zeta(3)/5 + 2*log(φ)^3/3 - 2*pi^2*log(φ)/15
        end
